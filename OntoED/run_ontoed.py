...
MODEL_CLASSES = {
    "bert": (BertConfig, OntoED, BertTokenizer),
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            inputs = {
                ...
                "labels": batch[3],
                "example_ids": batch[4],
                "rel_example_ids": batch[5],
                "rel_label_ids": batch[6],
            }
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    if (
                        ...
                        if results["eval_f1_micro"] > best_valid_f1_micro:
                            best_valid_f1_micro = results["eval_f1_micro"]
...
def evaluate(args, model, tokenizer, prefix="", test=False, infer=True):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        p_micro, r_micro, f1_micro, p_macro, r_macro, f1_macro = calculate_scores(preds, out_label_ids, len(processors[eval_task]().get_labels()))
        if infer:
            np.save(os.path.join(eval_output_dir, str(prefix)+"_preds.npy"),preds)
            
        result = {"eval_p_micro": p_micro, "eval_r_micro": r_micro, "eval_f1_micro": f1_micro, "eval_p_macro": p_macro, "eval_r_macro": r_macro, "eval_f1_macro": f1_macro, "eval_loss": eval_loss}
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False, test=False):
    ...
    all_rel_example_ids = torch.tensor([f.rel_example_ids.cpu().detach().numpy() for f in features], dtype=torch.long).to(torch.device("cuda"))
    all_rel_label_ids = torch.tensor([f.rel_label_ids.cpu().detach().numpy() for f in features], dtype=torch.long).to(torch.device("cuda"))

    dataset = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_label_ids, all_example_id, all_rel_example_ids, all_rel_label_ids)
    return dataset
