...
class EDModel(Model):
    def __init__(self, hyps, device=torch.device("cpu"), embeddingMatrix=None):
        ...
        for i in range(hyps["gcn_layers"]):
            gcn = GraphConvolution(in_features=2 * hyps["lstm_dim"],
                                   ...
                                   dropout=hyps["gcn_dp"] if i != hyps["gcn_layers"] -
                                   1 else None,
        ...
        if hyps["use_highway"]:
            ...
            for i in range(hyps["gcn_layers"]):
                hw = HighWay(size=2 * hyps["lstm_dim"],
                             dropout_ratio=hyps["gcn_dp"])
                self.hws.append(hw)

        # self attention
        self.sa = AttentionLayer(
            D=2 * hyps["lstm_dim"], H=hyps["sa_dim"], return_sequences=False)

        # Output Linear
        self.ol = BottledXavierLinear(
            in_features=2 * hyps["lstm_dim"], out_features=hyps["oc"]).to(device=device)

        # AE Output Linear
        self.ae_ol = BottledXavierLinear(
            in_features=4 * hyps["lstm_dim"], out_features=hyps["ae_oc"]).to(device=device)

        # Move to right device
        self.to(self.device)

    def get_sentence_positional_feature(self, BATCH_SIZE, SEQ_LEN):
        positions = [[abs(j) for j in range(-i, SEQ_LEN - i)]
                     for i in range(SEQ_LEN)]  # list [SEQ_LEN, SEQ_LEN]
        # list of tensors [SEQ_LEN]
    ...
    def forward(self, word_sequence, x_len, pos_tagging_sequence, entity_type_sequence, adj, batch_golden_entities,
        ...
        positional_sequences = self.get_sentence_positional_feature(
            BATCH_SIZE, SEQ_LEN)
        # (batch_size, seq_len, d)
        x_emb = torch.cat([word_emb, pos_emb, entity_label_emb], 2)
        # _ = x_emb, positional_sequences  # MOD
        xx = []

        for i in range(SEQ_LEN):
            # encoding
            # x, _ = self.bilstm(word_emb, x_len)  # MOD
            ...
            for i in range(self.hyperparams["gcn_layers"]):
                if self.hyperparams["use_highway"]:
                    # (batch_size, seq_len, d')
                    x = self.gcns[i](x, adj) + self.hws[i](x)
                else:
                    x = self.gcns[i](x, adj)
            ...
            # self attention
            # xx.append(self.sa(x, mask))  # (batch_size, d')  # MOD
        # output linear
        # xx = torch.stack(xx, dim=1)  # (batch_size, seq_len, d')  # MOD
        logits = self.ol(xx)

        ae_logits_key = []
        ae_hidden = []
        trigger_outputs = torch.max(logits, 2)[1].view(
            logits.size()[:2])  # (batch_size, seq_len)
        for i in range(BATCH_SIZE):
            ...
            for j in range(len(golden_entities)):
                ...
                try:
                    golden_entity_tensors[golden_entities[j]
                                          ] = xx[i, e_st:e_ed, ].mean(dim=0)  # (d')
                except:
                    print(xx.size())
                    print(e_st, e_ed)
            ...
            for st in predicted_event_triggers:
                ...
                for j in range(len(golden_entities)):
                    ...
                    ae_hidden.append(
                        torch.cat([event_tensor, entity_tensor]))  # (2 * d')
                    ae_logits_key.append(
                        (i, st, ed, trigger_type_str, e_st, e_ed, e_type_str))
        if len(ae_hidden) != 0:
    ...
    def calculate_loss_ed(self, logits, mask, label, weight):
        ...
        # mask = mask.view(BATCH * SEQ_LEN, )  # MOD
        mask = (label != 0).view(-1)
        masked_index = torch.LongTensor(
            [x for x in range(BATCH * SEQ_LEN) if mask[x] == 1]).to(self.device)
        output_ = output.index_select(0, masked_index)
        label_ = label.index_select(0, masked_index).squeeze(1)
        if weight is not None:
            weight = weight.to(self.device)
            loss = F.nll_loss(F.log_softmax(output_, dim=1),
                              label_, weight=weight)
        else:
    ...
    def calculate_loss_ae(self, logits, keys, batch_golden_events, BATCH_SIZE):
        ...
        for i, st, ed, event_type_str, e_st, e_ed, entity_type in keys:
            ...
            # if event matched
            if (st, ed, event_type_str) in batch_golden_events[i]:
        ...
        for (i, st, ed, event_type_str, e_st, e_ed, entity_type), ae_label in zip(keys, output_ae):
            ...
            predicted_events[i][(st, ed, event_type_str)].append(
                (e_st, e_ed, ae_label))
