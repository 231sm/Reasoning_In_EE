...
from enet.util import *


def train(model, train_set, dev_set, test_set, optimizer_constructor, epochs, tester, parser, other_testsets):
    # build batch
    torch.manual_seed(0)
    weighted_subsample(train_set)
    train_iter = BucketIterator(train_set, batch_size=parser.batch, train=False, shuffle=True, device=model.device,
                                sort_key=lambda x: len(x.POSTAGS))
    dev_iter = BucketIterator(dev_set, batch_size=parser.batch, train=False, shuffle=False, device=model.device,
                              sort_key=lambda x: len(x.POSTAGS))
    test_iter = BucketIterator(test_set, batch_size=parser.batch, train=False, shuffle=False, device=model.device,
                               sort_key=lambda x: len(x.POSTAGS))
...
def test(model, test_set, tester, parser):
    test_iter = BucketIterator(test_set, batch_size=parser.batch, train=False, shuffle=False, device=model.device,
                               sort_key=lambda x: len(x.POSTAGS))
    run_over_data(data_iter=test_iter,
                  optimizer=None,
                  model=model,
                  need_backward=False,
                  MAX_STEP=ceil(
                  ...
                  role_i2s=parser.role_i2s,
                  weight=parser.label_weight,
                  save_output=os.path.join(parser.out, "test.txt"))
