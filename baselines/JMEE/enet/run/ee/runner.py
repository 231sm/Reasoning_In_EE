...
from enet.training import train, test
...
class EERunner(object):
    ...
    def run(self):
        ...
        EntitiesField = EntityField(
            lower=False, batch_first=True, use_vocab=False)
        ...
        if self.a.webd:
            pretrained_embedding = Vectors(self.a.webd, ".", unk_init=partial(
                torch.nn.init.uniform_, a=-0.15, b=0.15))
            WordsField.build_vocab(
                train_set.WORDS, dev_set.WORDS, vectors=pretrained_embedding)
        else:
            WordsField.build_vocab(train_set.WORDS, dev_set.WORDS)
        PosTagsField.build_vocab(train_set.POSTAGS, dev_set.POSTAGS)
        EntityLabelsField.build_vocab(
            train_set.ENTITYLABELS, dev_set.ENTITYLABELS)
        ...
        if "psemb_size" not in self.a.hps:
            self.a.hps["psemb_size"] = max(
                [train_set.longest(), dev_set.longest(), test_set.longest()]) + 2
